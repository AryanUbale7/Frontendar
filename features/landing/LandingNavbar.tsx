        ...
        <nav className="hidden lg:flex items-center gap-6">
          {LANDING_NAV_LINKS.map((link) => (
            <a
              key={link.label}
              href={link.href}
              className="text-xs font-semibold text-slate-605 hover:text-[#DEE32E] transition-colors"
            >
              {link.label}
            </a>
          ))}
        ...
        <div className="lg:hidden border border-slate-200/50 bg-white/95 backdrop-blur-md p-4 mt-2 rounded-2xl shadow-lg space-y-3 animate-in slide-in-from-top-2 pointer-events-auto">
          {LANDING_NAV_LINKS.map((link) => (
            <a
              ...
              className="block py-1.5 text-xs font-semibold text-slate-600 hover:text-[#DEE32E]"
            >
              {link.label}
            </a>
          ))}
