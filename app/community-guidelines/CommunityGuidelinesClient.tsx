...
import { GUIDELINES_CONFIG, GUIDELINES_SECTIONS } from "@/constants/community-guidelines";

export function CommunityGuidelinesClient() {
  const [activeSectionId, setActiveSectionId] = useState<string>(GUIDELINES_SECTIONS[0].id);
  ...
  useEffect(() => {
    ...
    GUIDELINES_SECTIONS.forEach((sec) => {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#F8FAFC] text-[#0F172A] selection:bg-[#2563EB]/20 selection:text-[#2563EB]">
      ...
      <section className="relative overflow-hidden border-b border-[#E2E8F0] bg-white py-16 md:py-20 shadow-2xs">
        ...
        <div className="absolute top-0 left-0 right-0 h-1.5 bg-gradient-to-r from-[#0F172A] via-[#FF8A00] to-[#FFD60A]" />
        ...
        <div className="mx-auto max-w-7xl px-4 md:px-6 relative z-10">
          <div className="flex flex-col items-center text-center max-w-3xl mx-auto space-y-5">
            ...
            <div className="inline-flex items-center gap-2 rounded-full border border-[#2563EB]/20 bg-[#2563EB]/10 px-3.5 py-1 text-xs font-bold uppercase tracking-wider text-[#2563EB] shadow-2xs">
              ...
              <span>{GUIDELINES_CONFIG.brandName}</span>
            </div>
            ...
            {/* Subtitle / Principle Banner */}
            <p className="text-base sm:text-lg text-[#475569] font-medium leading-relaxed">
              Design. Build. Dominate. — Together. Building a respectful, fair, and collaborative developer ecosystem.
            </p>
            ...
            <div className="flex items-center gap-2.5 text-xs font-semibold text-[#334155] bg-[#F1F5F9] border border-[#E2E8F0] px-4 py-2 rounded-xl">
              ...
              <span>Last Updated: {GUIDELINES_CONFIG.lastUpdated}</span>
            </div>
          </div>
        </div>
      </section>
              ...
              >
                ...
                <div className="space-y-4 text-[#334155] text-sm md:text-base leading-relaxed font-medium">
                  {sec.content.map((block, idx) => {
                    ...
                    if (block.type === "highlight_banner") {
                      return (
                        <div
                          key={idx}
                          className="my-4 rounded-xl border border-amber-300 bg-amber-50 p-4 text-[#0F172A] font-extrabold text-base md:text-lg flex items-center gap-3 shadow-2xs"
                        >
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-[#334155]">
                            <div>
                              <span className="text-[#64748B] block text-[11px] uppercase tracking-wider font-semibold">Tagline</span>
                              <span className="font-bold text-[#0F172A]">{GUIDELINES_CONFIG.tagline}</span>
                            </div>
                            <div>
                              <span className="text-[#64748B] block text-[11px] uppercase tracking-wider font-semibold">Contact Email</span>
                              <a
                                href={`mailto:${GUIDELINES_CONFIG.officialEmail}`}
                                className="font-bold text-[#2563EB] hover:underline"
                              >
                                {GUIDELINES_CONFIG.officialEmail}
                              </a>
                            </div>
                            <div className="sm:col-span-2">
                              <span className="text-[#64748B] block text-[11px] uppercase tracking-wider font-semibold">Website</span>
                              <a
                                href={GUIDELINES_CONFIG.websiteUrl}
                                target="_blank"
                                rel="noreferrer"
                                className="font-bold text-[#2563EB] hover:underline flex items-center gap-1"
                              >
                                {GUIDELINES_CONFIG.websiteUrl}
            ...
            <div className="rounded-2xl border border-[#2563EB]/20 bg-gradient-to-r from-white via-[#F8FAFC] to-[#2563EB]/5 p-8 shadow-sm flex flex-col sm:flex-row items-center justify-between gap-6">
              <div className="space-y-1 text-center sm:text-left">
                ...
                <h3 className="font-heading text-lg font-bold text-[#0F172A]">
                  Have a question about our Community Guidelines?
                </h3>
                <p className="text-xs text-[#475569]">
                  Contact the Frontend Arena team for any reports or inquiries.
                </p>
              </div>
          ...
          <aside className="hidden lg:block lg:col-span-4 sticky top-24 space-y-4">
            ...
            <div className="rounded-xl border border-[#E2E8F0] bg-white p-4 text-[11px] text-[#64748B] space-y-2 shadow-2xs">
              <div className="flex items-center gap-1.5 text-[#0F172A] font-bold">
                <HeartHandshake className="h-3.5 w-3.5 text-[#2563EB]" />
                <span>{GUIDELINES_CONFIG.tagline}</span>
              </div>
              <p>
                © {new Date().getFullYear()} {GUIDELINES_CONFIG.brandName}. All rights reserved.
              </p>
            </div>
          </aside>
        </div>
