...
exports.FAIEOrchestrator = void 0;
const knowledge_engine_1 = require("./knowledge-engine/knowledge.engine");
const synonym_engine_1 = require("./synonym-engine/synonym.engine");
const feature_engine_1 = require("./feature-engine/feature.engine");
const repository_engine_1 = require("./repository-engine/repository.engine");
const route_engine_1 = require("./route-engine/route.engine");
const ui_engine_1 = require("./ui-engine/ui.engine");
const inference_engine_1 = require("./inference-engine/inference.engine");
const confidence_engine_1 = require("./confidence-engine/confidence.engine");
const reasoning_engine_1 = require("./reasoning-engine/reasoning.engine");
const evidence_engine_1 = require("./evidence-engine/evidence.engine");
const scoring_engine_1 = require("./scoring-engine/scoring.engine");
class FAIEOrchestrator {
    knowledgeEngine = new knowledge_engine_1.KnowledgeEngine();
    synonymEngine;
    featureEngine;
    repositoryEngine = new repository_engine_1.RepositoryEngine();
    routeEngine = new route_engine_1.RouteEngine();
    uiEngine = new ui_engine_1.UIEngine();
    inferenceEngine = new inference_engine_1.InferenceEngine();
    confidenceEngine;
    reasoningEngine = new reasoning_engine_1.ReasoningEngine();
    evidenceEngine = new evidence_engine_1.EvidenceEngine();
    scoringEngine = new scoring_engine_1.ScoringEngine();
    constructor(customSynonymDict, confidenceThreshold = 75) {
        this.synonymEngine = new synonym_engine_1.SynonymEngine(customSynonymDict);
        this.featureEngine = new feature_engine_1.FeatureEngine(this.synonymEngine, confidenceThreshold);
        this.confidenceEngine = new confidence_engine_1.ConfidenceEngine(confidenceThreshold);
    }
    async evaluate(workspacePath, repoUrl, blueprint, deploymentUrl) {
        const logs = [];
        ...
        const featureResults = this.featureEngine.evaluateFeatures(workspacePath, normalizedFeatures, repoAnalysis, routeResults, this.uiEngine.analyzeUI(workspacePath, deploymentUrl));
        // Collect all rejected claims across features
        const allRejectedClaims = [];
        ...
        blueprint.scoringSystem.categories.forEach((cat) => {
            ...
            reasonings.push(this.reasoningEngine.generateReasoning(cat.name, scoreAwarded, cat.maxMarks, cat.passingMarks, ruleApplied, reasonStr, confidence, this.evidenceEngine.getAllEvidence(), routeResults.detectedRoutes.map((r) => r.filePath), routeResults.detectedRoutes.map((r) => r.pattern), repoAnalysis.detectedComponents, uiAnalysis.detectedUIComponents));
        });
        // 9. Scoring Engine
        logs.push(`[FAIE v2 10/12] Scoring Engine: Computing final weighted scores...`);
        const faieSummary = this.scoringEngine.calculateFinalScores(blueprint, featureResults, repoAnalysis, routeResults, uiAnalysis, reasonings);
...
exports.FAIEOrchestrator = FAIEOrchestrator;
